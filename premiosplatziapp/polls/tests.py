...
class QuestionModelTest(TestCase):  # PARA TESTEAR MODELOS
    """
    CONSTRUIMOS UNA BATERÍA DE TESTS QUE SON LAS PRUEBAS QUE VAMOS A HACER A NUESTRO MODELO
    """

    def setUp(self) -> None:
        """ setUp() es un método que se llama antes de cada test """
        # Creamos una pregunta
        self.question = Question.objects.create(question_text="¿Quién es el mejor CD de Platzi?",
                                                pub_date=timezone.now())

    def test_was_published_recently_with_future_questions(self):
        """was_published_recently() returns False for questions whose pub_date is in the future."""
        future = timezone.now() + datetime.timedelta(days=30)
        self.question.pub_date = future
...
class QuestionIndexViewTests(TestCase):  # PARA TESTEAR VISTAS

    def test_no_questions(self):
        """If no questions exist, an appropriate message is displayed."""
        response = self.client.get(reverse("polls:index"))
        # self.client.get() nos permite hacer una petición a una url
        # reverse() nos permite obtener la url de la vista que queremos testear
...
class QuestionResultsViewTests(TestCase):
    ...
    def test_for_question_not_exist(self):
        """
        If the id for a question does not exist, the results view returns a 404 not found.
        """

        response = self.client.get(reverse("polls:results", args=(8,)))
        self.assertEqual(response.status_code, 404)
